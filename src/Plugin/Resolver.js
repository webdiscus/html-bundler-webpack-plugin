...
class Resolver {
   ...
   *
   * @type {Map<string, {assets:Map, originalAssetFile:string, moduleHandler?:(originalAssetFile:string, issuer:string) => string}>}
   */
  static data = new Map();
   ...
   * @param {Object} entryPoint The current entry point.
   */
  static addAsset(sourceFile, assetFile, entryPoint) {
    ...
    if (!item) {
      ...
      this.data.set(sourceFile, resource);
      return;
    }
   ...
   *
   * Note: used for 'responsive-loader'.
   *
   * @param {string} sourceFile The full path of source asset file.
   ...
   * @param {Object} entryPoint The current entry point.
   */
  static addResolvedAsset(sourceFile, assetFile, issuer, entryPoint) {
    const assetKey = this.getAssetKey(issuer, entryPoint);
    sourceFile = path.resolve(sourceFile);

    if (Options.isPreload()) {
      const preloadAssetFile = this.getPreloadAssetFile({ assetFile, issuer, entryPoint });
      Preload.add(entryPoint.filename, { sourceFile, assetFile: preloadAssetFile });
    }
    ...
    if (!item) {
      const resource = {
        sourceFile,
        assets: new Map([[assetKey, assetFile]]),
      };
      this.data.set(sourceFile, resource);
      return;
    }
    item.assets.set(assetKey, assetFile);
  }

  /**
   * Get preload output file of an asset relative by entry point.
   *
   ...
   * @return {string}
   */
  static getPreloadAssetFile({ assetFile, issuer, entryPoint }) {
    let preloadAssetFile = assetFile;

    if (issuer !== entryPoint.request) {
      const issuerAssetFile = Asset.findAssetFile(issuer);
      // recovery web root path of assetFile when issuer is not the entry, e.g. a style
      const issuerDir = path.dirname(issuerAssetFile);
      const webRootPath = path.posix.join(issuerDir, assetFile);
      preloadAssetFile = Options.getAssetOutputFile(webRootPath, entryPoint.filename);
    }

    return preloadAssetFile;
  }
  ...
  static resolveAsset(sourceFile) {
    ...
    if (originalAssetFile != null) {
      ...
      if (AssetInline.isDataUrl(originalAssetFile)) {
        ...
        let preloadAssetFile = Options.getAssetOutputFile(originalAssetFile, entryPoint.filename);

        if (isInlinedIssuer) {
          assetOutputFile = preloadAssetFile;
        } else {
          const issuerAssetFile = Asset.findAssetFile(issuer);
          if (issuerAssetFile) {
        ...
        Options.isPreload() && Preload.add(entryPoint.filename, { sourceFile, assetFile: preloadAssetFile });
      }
      ...
      if (Options.isPreload()) {
        const preloadAssetFile = this.getPreloadAssetFile({ assetFile: assetOutputFile, issuer, entryPoint });
        Preload.add(entryPoint.filename, { sourceFile, assetFile: preloadAssetFile });
      }
    }

    if (assetOutputFile != null) {
      item.assets.set(assetKey, assetOutputFile);
    }
  ...
  static require(rawRequest) {
    ...
    if (sourceFile != null) {
      const assetFile = this.resolveAsset(sourceFile);
