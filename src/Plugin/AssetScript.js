        ...
        for (const issuerAssetFilename of issuer.assets.keys()) {
          ...
          if (chunkFiles.length === 1) {
            ...
            if (asset.inline === true) {
              ...
              // supports for preload a script in link tag and load in script tag under same name
              if (String.prototype.replaceAll) {
                // works in node.js >= 15
                newContent = content.replaceAll(sourceFile, assetFile);
              } else {
                // TODO: remove polyfill when support for node.js 14 is dropped
                newContent = content.replace(new RegExp(sourceFile, 'g'), assetFile);
              }
              ...
              Options.isPreload() && Preload.add(issuerAssetFilename, { sourceFile, assetFile });
            }
          } else {
            ...
            if (scriptTags) {
              ...
              const nextChar = content[tagEndPos];
              if ('\n\r'.indexOf(nextChar) < 0) scriptTags += LF;
