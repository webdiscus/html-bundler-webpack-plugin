...
const { referencesProps } = require('svgo/plugins/_collections.js');

exports.name = 'uniqueIds';
exports.description = 'prefix with unique IDs';
...
exports.fn = (_root, params, info) => {
  ...
  const id = params.uniqueId !== false ? `-${uid++}` : '';
  ...
  return {
    element: {
      enter: (node) => {
         ...
         * @type {string}
         */
        let prefix = 'prefix' + id + delim;
        if (typeof params.prefix === 'function') {
          prefix = params.prefix(node, info) + id + delim;
        } else if (typeof params.prefix === 'string') {
          prefix = params.prefix + id + delim;
        } else if (params.prefix === false) {
          prefix = params.uniqueId !== false ? `id${id}${delim}` : '';
        } else if (info.path != null && info.path.length > 0) {
          prefix = escapeIdentifierName(getBasename(info.path)) + id + delim;
        }
